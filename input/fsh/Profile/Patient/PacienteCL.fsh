...
* identifier ^slicing.description = "Se diferencia el RUN de otros identificadores"
* identifier contains RUN 0..1 MS

* identifier ^short = "Listados de Id de Paciente. De poseer una CI con RUN vigente, este DEBE ser ingresado en el Slice Asignado"
...
* identifier.type.extension contains PaisEmisionDoc named PaisEmisor 0..1  MS
...
* identifier[RUN] only $deis.id
* identifier[RUN] ^short = "Identificador de Paciente con RUN"
...
//* identifier.use ^definition = "Se definirá este uso siempre como \"official\" debido a que cualquier ID presentado para motivos de este perfil debe ser de este tipo"
...
* name[NombreOficial] ^definition = "Determinación del nombre registrado oficialmente del Paciente"
  ...
  * family  MS
  * family.extension contains https://interoperabilidad.minsal.cl/fhir/ig/eis/StructureDefinition/SegundoApellido named segundoApellido 0..1 MS
...
* name[NombreSocial] ^short = "Nombre con el cual se identifica al paciente sin ser este oficial. Se especifica solo en el uso del nombre"
* name[NombreSocial] ^definition = "Nombre con el cual se identifica al paciente sin ser este oficial. Se especifica solo en el uso del nombre"
...
* telecom ^definition = "Detalles del contacto de un paciente comúnmente el o los más usados (Ej: Teléfono fijo, móvil, email, etc.)"
  ...
  * value ^short = "Dato del contacto del paciente descrito"
  * value ^definition = "Valor del contacto como por ejemplo el número de teléfono fijo o de móvil o el email del Paciente"
...
* contact ^definition = "Contacto, tutor legal o representante del Paciente"
  ...
  * name ^definition = "Nombre del contacto asociado al paciente"
    ...
    * family ^definition = "Se define el primer apellido registrado al momento de nacer o aquel que se ha inscrito legalmente en el Registro Civil"
    * family 0..1 MS
      * extension contains https://interoperabilidad.minsal.cl/fhir/ig/eis/StructureDefinition/SegundoApellido named segundoApellido 0..1 MS
      //* extension 0..1 MS
      * extension ^short = "Extensión para 2o apellido"
      * extension ^definition = "Extensión para la declaración de un segundo apellido"
    * given MS
